...
def create_crypto_data_pipeline(con: duckdb.DuckDBPyConnection) -> Dict[str, TableConfig]:
    ...
    table_configs = {
        ...
        'bn_premium': TableConfig(
            ...
            fetch_args={},
            update_frequency='1m',  # Funding rates update every 8 hours
            needs_incremental=True
        ),
...
class CryptoDataPipeline:
    ...
    def update_table(self, config: TableConfig, new_df: pd.DataFrame):
        ...
        update_columns = [col for col in config.columns.keys() if col not in config.primary_keys]
        if update_columns:
            update_stmt = f"""
                UPDATE {config.name} 
    ...
    def update_all(self, start_time: Optional[datetime] = None, end_time: Optional[datetime] = None):
        ...
        for table_name in ['bn_perp_klines','bn_spot_klines','bn_premium']:
            try:
                print(f"\nUpdating {table_name}...")
                self.update_market_data(self.table_configs[table_name], start_time, end_time)
            except Exception as e:
...
#fetcher.fetch_market_margin_interest_rates_threadpool(start_time=pd.Timestamp('2025-02-21 00:00:00'), end_time=pd.Timestamp.now(tz='UTC'))
